...
cat /tmp/1.json | sed "s/^.*\"rpc\": .*/    \"rpc\": [\"https:\/\/${NETNAME}.rpc${NETDOMAIN}\"],/g" > ./src/chains/mainnet/agoric.json

cat vue.config.js | sed "s/devServer: {/devServer: {disableHostCheck: true,/g" > /tmp/v.js
